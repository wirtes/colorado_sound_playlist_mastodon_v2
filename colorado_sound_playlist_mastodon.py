...
def get_now_playing_from_api(url):
    # Grab it from https://api.composer.nprstations.org/widget/{ucs}/now
    response = requests.get(url)
    if response.status_code == 200:
        npr_response = json.loads(response.content)
        song = npr_response["onNow"]
    return song


def build_current_song(song):
    current_song = {}
    current_song["artistName"] = song["song"]["artistName"]
    current_song["trackName"] = song["song"]["trackName"]
    current_song["collectionName"] = song["song"]["collectionName"]
    current_song["dj"] = song["program"]["name"]
    current_song["itunes_link"] = song["song"]["buy"]["itunes"]
    current_song["start_time"] = song["start_time"]
    current_song['cover_art_available'] = True
    # current_song["is_song"] = True
    current_song["id"] = song["song"]["artistName"] + song["song"]["trackName"]
...
def call_api_and_post():
    song = get_now_playing_from_api(config["api_url"])
    current_song = build_current_song(song)
